...
public class TaskEditServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        this.addUserParameters(userBean.getCurrentUser(), task, session);
                                
        request.setAttribute("readonly", !this.userBean.getCurrentUser().getUsername().equals(task.getOwner().getUsername()));
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Task task = this.getRequestedTask(request);
        if(task.getOwner() == null || !task.getOwner().getUsername().equals(this.userBean.getCurrentUser().getUsername())) {
            return;
        }
    ...
    private void saveTask(HttpServletRequest request, HttpServletResponse response)
        ...
        String taskArt = request.getParameter("task_art");
        ...
        try {
            task.setPrice(Double.parseDouble(taskPreis));
        } catch(Exception e) {
            
        }

        try {
            task.setArt(Task.Art.valueOf(taskArt));
        } catch (IllegalArgumentException ex) {
            errors.add("Der ausgewählte Status ist nicht vorhanden.");
        }
        
        try {
            task.setPreisArt(Task.PreisArt.valueOf(taskPreisArt));
        } catch (IllegalArgumentException ex) {
            errors.add("Der ausgewählte Status ist nicht vorhanden.");
        }
    ...
    private void addUserParameters(User user, Task task, HttpSession session) {
        session.setAttribute("user", user);
        session.setAttribute("angelegt_am", task.getDueDate() == null ? new Date(System.currentTimeMillis()) : task.getDueDate());
    }
    ...
    private FormValues createTaskForm(Task task) {
        ...
        values.put("task_preis", new String[]{"" + task.getPrice()});
